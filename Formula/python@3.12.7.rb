class PythonAT3127 < Formula
  ...
  def lib_cellar
    on_macos do
      return frameworks/"Python.framework/Versions"/version_major_minor/"lib/python#{version_major_minor}"
    end
    on_linux do
      return lib/"python#{version_major_minor}"
    end
  end
  ...
  def site_packages
    HOMEBREW_PREFIX/"lib/python#{version_major_minor}/site-packages"
  end

  def python3
    bin/"python#{version_major_minor}"
  end
  ...
  def install
    ...
    if OS.mac?
      ...
      pc_dir = frameworks/"Python.framework/Versions"/version_major_minor/"lib/pkgconfig"
      ...
      bad_cellar_path_files = [
        ...
        lib_cellar/"config-#{version_major_minor}-darwin/Makefile",
        pc_dir/"python-#{version_major_minor}.pc",
        pc_dir/"python-#{version_major_minor}-embed.pc",
      ]
      inreplace bad_cellar_path_files, prefix, opt_prefix

      # Help third-party packages find the Python framework
      inreplace lib_cellar/"config-#{version_major_minor}-darwin/Makefile",
      ...
      inreplace Dir[lib_cellar/"**/_sysconfigdata_*linux_x86_64-*.py",
                    ...
                    bin/"python#{version_major_minor}-config",
    ...
    {
      "idle"           => "idle#{version_major_minor}",
      "idle3"          => "idle#{version_major_minor}",
      "pydoc"          => "pydoc#{version_major_minor}",
      "pydoc3"         => "pydoc#{version_major_minor}",
      "python"         => "python#{version_major_minor}",
      "python3"        => "python#{version_major_minor}",
      "python-config"  => "python#{version_major_minor}-config",
      "python3-config" => "python#{version_major_minor}-config",
  ...
  def post_install
    ...
    # Create a site-packages in HOMEBREW_PREFIX/lib/python#{version_major_minor}/site-packages
    ...
    rm_r(bin.glob("pip{,3}"))
    mv bin/"wheel", bin/"wheel#{version_major_minor}"
    ...
    %W[wheel#{version_major_minor} pip#{version_major_minor}].each do |e|
  ...
  def sitecustomize
    <<~EOS
      ...
      import site
      import sys
      if sys.version_info[:2] != (#{version_major}, #{version_minor}):
          ...
          exit(f'Your PYTHONPATH points to a site-packages dir for Python #{version_major_minor} '
      ...
      if os.path.realpath(sys.executable).startswith('#{rack}'):
          ...
          library_site = '/Library/Python/#{version_major_minor}/site-packages'
          ...
          long_prefix = re.compile(r'#{rack}/[0-9\\._abrc]+/Frameworks/Python\\.framework/Versions/#{version_major_minor}/lib/python#{version_major_minor}/site-packages')
          sys.path = [long_prefix.sub('#{site_packages}', p) for p in sys.path]
          # Set the sys.executable to use the opt_prefix. Only do this if PYTHONEXECUTABLE is not
          # explicitly set and we are not in a virtualenv:
          if 'PYTHONEXECUTABLE' not in os.environ and sys.prefix == sys.base_prefix:
              sys.executable = sys._base_executable = '#{opt_bin}/python#{version_major_minor}'
      ...
      for split_module in ["tk", "gdbm"]:
          split_prefix = f"#{HOMEBREW_PREFIX}/opt/python-{split_module}@#{version_major_minor}/libexec"
  ...
  def caveats
    <<~EOS
      Python is installed as
        #{HOMEBREW_PREFIX}/bin/python#{version_major_minor}

      Unversioned and major-versioned symlinks `python`, `python3`, `python-config`, `python3-config`, `pip`, `pip3`, etc. pointing to
      `python#{version_major_minor}`, `python#{version_major_minor}-config`, `pip#{version_major_minor}` etc., respectively, are installed into
        #{opt_libexec}/bin
  ...
  test do
    ...
    system bin/"pip#{version_major_minor}", "list", "--format=columns"
